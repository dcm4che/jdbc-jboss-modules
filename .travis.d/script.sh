...
then
  ...
  BUILD_VERSION="1.0.1-$(printf '%04d' ${TRAVIS_BUILD_NUMBER})$(date -u '+%Y%m%d%H%M%S')"
  ...
  mvn -s .travis.d/settings.xml -P ossrh-up,travis-secret,db-all deploy -Dscm.revision="${TRAVIS_COMMIT}"
else
